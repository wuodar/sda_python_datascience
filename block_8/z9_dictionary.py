...
def create_dict(input_string: str) -> Dict[str, int]:
    ...
    return my_dict
    # my_dictionary = {value: index + 1 for index, value in enumerate(input_string)}
...
def get_keys_with_largest_values(input_dict: dict[Any, float]) -> List[Any]:
    ...
    for i in sorted_values:
        for k in input_dict.keys():
            if input_dict[k] == i:
                sorted_dict[k] = input_dict[k] #sorted_dict[k] = i
                break
    sorted_keys = list(sorted_dict.keys())
    three_larges_keys = sorted_keys[:3]
    return three_larges_keys[::-1]
