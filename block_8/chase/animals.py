...
class Wolf(Animal):
    ...
    def __get_closest_sheep(self, sheeps: List[Sheep]) -> Tuple[int, float]:
        ...
        """
        distance = [dist(self.cords, sheep.cords) if sheep.alive else float("inf") for sheep in sheeps]
        minimun_distance = min(distance)
        closest_sheep_index = distance.index(minimun_distance)
        return closest_sheep_index, minimun_distance
