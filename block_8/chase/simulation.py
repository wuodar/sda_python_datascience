...
def count_alive_sheeps(sheeps: List[Sheep]) -> int:
    caunt_alive_sheeps = [1 if sheep.alive else 0 for sheep in sheeps]
    return sum(caunt_alive_sheeps)
...
def run_simulation(iterations_count: int, wolf: Wolf, sheeps: List[Sheep]):
    ...
    while iterations_count >= i and count_alive_sheeps(sheeps) > 0:
        ...
        i += 1
    logging.info(f"Simulation finished! There are {count_alive_sheeps(sheeps)} alive sheeps!")
